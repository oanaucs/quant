...
tf.app.flags.DEFINE_integer(
    'test_image_size', 224, 'Train image size')

##############
# Test Flags #
##############

tf.app.flags.DEFINE_string('checkpoint_path', '.\\..\\..\\trained_models\\vgg_quant\\16_quant_model.ckpt-50',
    ...
    with session.as_default():
        ...
        try:
            for i in range(0, num_batches):
                start = time.time()
                preds = session.run(fetches=predictions)
